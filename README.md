...
- In je main methode staan twee arrays (1 numeriek en 1 alfabetisch), een boolean variabele, een novi.nl.Translator object en een Scanner object;
- Je project bevat 1 novi.nl.Translator class met daarin een HashMap variabele, een constructor met 2 arrays als parameter en een translate functie;
- De logica van de applicatie wordt gedraaid in een while(boolean)-loop in je main methode.

## Stappenplan
Let op: het is uitdagender om jouw eigen stappenplan te maken. Als je niet zo goed weet waar je moet beginnen, kun je onderstaand stappenplan gebruiken:
1. Open de novi.nl.Main class in de `src`-map. Als het goed is zie je hier een `public static void main (String[] args)` methode.
...
4. Maak een nieuwe class aan en noem deze `novi.nl.Translator`.
5. Maak in de novi.nl.Translator class een `HashMap<Integer,String>` variabele met de naam `numericAlpha`.
6. Maak in de novi.nl.Translator class een constructor die de volgende parameters krijgt: `(String[] alphabetic, Integer[] numeric)`.
...
9. De constructor is klaar. Nu ga je deze aanroepen met de juiste argumenten in de main methode van de novi.nl.Main class, oftewel: maak in main een nieuw object aan van het type novi.nl.Translator.
10. Maak in de novi.nl.Translator class een nieuwe methode genaamd `translate(Integer number)` die een String returnt.
...
12. Maak in de main methode van de novi.nl.Main class een boolean variabele genaamd `play` met de waarde `true`. Maak een String variabele genaamd `ongeldig` waarin je de String "ongeldige invoer" zet. Als laatst maak je nog een nieuw Scanner object aan met `Scanner scanner = new Scanner(System.in)`.
...
**novi.nl.Bonus-bonus:** maak de methode zo dat je vaker kan raden.
